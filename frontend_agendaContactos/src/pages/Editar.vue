...
const cargarContacto = async () => {
    try {
        const response = await axios.get(`http://localhost:8080/contactos/traerContacto/${id}`);
...
const actualizarContacto = async () => {
    ...
    await axios.put(`http://localhost:8080/contactos/editarContacto/${id}`, nuevoContacto.value);
...
<template>
    ...
    <div class="container my-5">
        <h2 class="text-center fw-bold mb-4">Editar Contacto</h2>
        <form @submit.prevent="actualizarContacto" class="mx-auto" style="max-width: 600px;">
            <div class="mb-3">
            ...
            <button type="submit" class="btn btn-primary w-100">Actualizar Contacto</button>
        </form>
    </div>
